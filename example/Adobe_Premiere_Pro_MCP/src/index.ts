...
import { OAuthValidator } from './auth/oauth-validator.js';
import type { AuthUser } from 'auth-agent-mcp-sdk';
...
class MCPPremiereProServer {
  ...
  constructor() {
    ...
    if (process.env.OAUTH_SERVER_ID && process.env.OAUTH_API_KEY) {
      const config: any = {
        serverId: process.env.OAUTH_SERVER_ID,
        apiKey: process.env.OAUTH_API_KEY,
        requiredScopes: ['premiere:edit'],
      };

      if (process.env.OAUTH_SERVER_URL) {
        config.authServerUrl = process.env.OAUTH_SERVER_URL;
      }

      this.oauthValidator = new OAuthValidator(config);
      this.logger.info('OAuth authentication enabled');
    } else {
      this.logger.warn('OAuth not configured - running without authentication');
    }

    this.setupHandlers();
  }

  private async validateOAuth(request: any): Promise<AuthUser | null> {
    if (!this.oauthValidator?.isEnabled()) {
      return null; // OAuth not configured, skip validation
    }

    // Extract token from request meta (MCP spec allows meta field)
    const token = request.params?._meta?.authorization?.replace('Bearer ', '');

    if (!token) {
      throw new McpError(
        ErrorCode.InvalidRequest,
        'OAuth authentication required. Please provide Authorization token.'
      );
    }

    try {
      const user = await this.oauthValidator.validateToken(token);
      return user;
    } catch (error) {
  ...
  private setupHandlers(): void {
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      const user = await this.validateOAuth(request);
      if (user) {
        this.logger.info(`Tool '${name}' called by authenticated user: ${user.email}`);
      }
    ...
    this.server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
      ...
      const user = await this.validateOAuth(request);
      if (user) {
        this.logger.info(`Resource '${uri}' accessed by authenticated user: ${user.email}`);
      }
    ...
    this.server.setRequestHandler(GetPromptRequestSchema, async (request) => {
      ...
      const user = await this.validateOAuth(request);
      if (user) {
        this.logger.info(`Prompt '${name}' requested by authenticated user: ${user.email}`);
      }
